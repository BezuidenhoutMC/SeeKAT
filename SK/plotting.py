...
def likelihoodPlot(f,ax,loglikelihood,options):
    ...
    if options.sb[0] != 0:
        scalebar = AnchoredSizeBar(ax.transData,
                           int(options.sb[0]/options.res[0]), '%d arcsec' % (options.sb[0]), 'upper left', 
                           pad=1.0,
                           color='black',
                           frameon=True,
                           size_vertical=0.2)
    ...
    plt.scatter(np.where(likelihood==np.amax(likelihood))[1],np.where(likelihood==np.amax(likelihood))[0],marker='x',c='red')
    ...
    if len(np.nonzero(likelihood_flat_sorted_cumsum > (1-0.9545))[0]) != 0:
        ...
        CS2 = plt.contour(likelihood,levels=[val_2sigma],zorder=800,linestyles='--',colors='red')
    else:
        val_2sigma = np.nan
