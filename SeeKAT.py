...
def make_map(array_height, array_width, c, psf_ar, options, data):
    ...
    for j in range(1, npairs + 1):
        ...
        plt.contour(
            comparison_ar, levels=[options.overlap], colors="black", linewidths=0.5
        )
    ...
    chi2 = np.sum(
        resids
        * np.sum(np.linalg.inv(C)[None, None, :, :] * resids[:, :, :, None], axis=2),
        axis=2,
    )
...
if __name__ == "__main__":
    ...
    if options.autozoom == True:
        ax.set_xlim(
            min(c.ra.px) - int(15 / options.res[0]),
            max(c.ra.px) + int(15 / options.res[0]),
        )
        ax.set_ylim(
            min(c.dec.px) - int(15 / options.res[0]),
